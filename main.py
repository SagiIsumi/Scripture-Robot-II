...
async def input_trans(MyAudio:object):
    task1 = asyncio.create_task(MyAudio.check_language(input))
    task2=asyncio.create_task(MyAudio.speech_to_text(input))
    responses = await asyncio.gather(task1,task2)
    print(responses)
    if responses[1]=='error':
        results = responses[0]
    else:
        results = (responses[0][0],responses[1])
    return results
...
if __name__=="__main__":
    ...
    while True:
        ...
        language, text  = asyncio.run(input_trans(MyAudio))
        print("text: ",text)
